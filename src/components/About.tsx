...
import { FolderGit2, Award, Cpu, CheckCircle2, Sparkles, MapPin, GraduationCap } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          >
            ...
            <div className="p-5 rounded-2xl bg-[#FCFAF6] border border-[#DDD6C8] shadow-sm space-y-3.5">
              <div className="flex items-center space-x-3 text-sm text-[#1D2A26]">
                ...
                <span className="text-[#4B5563]">{personalInfo.location}</span>
              </div>
              <div className="flex items-center space-x-3 text-sm text-[#1D2A26]">
                <GraduationCap className="w-4 h-4 text-[#D97745]" />
                <span className="font-semibold">Education:</span>
                <span className="text-[#4B5563]">BSc Statistics, University of Ilorin</span>
              </div>
              <div className="flex items-center space-x-3 text-sm text-[#1D2A26]">
                <Cpu className="w-4 h-4 text-[#4E8D66]" />
                <span className="font-semibold">Core Focus:</span>
                <span className="text-[#4E8D66] font-mono text-xs font-semibold">{personalInfo.focusArea}</span>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="lg:col-span-6 space-y-4 text-[#4B5563] text-base leading-relaxed"
          >
            <p>
              I am an <strong className="text-[#1D2A26]">AI Automation Developer & Data Analyst</strong> with a strong background in Statistics and a passion for constructing intelligent solutions that simplify complex operational workflows.
            </p>
            <p>
              I specialize in designing <strong className="text-[#1D2A26]">Google Workspace automations</strong>, Google Apps Script serverless routines, custom PDF generators, and data-driven analytical models that help businesses save hundreds of manual hours.
            </p>
            <p>
              My analytical experience spans Microsoft Excel, SQL, Power BI, Google Apps Script, and AI-powered workflow automation. I enjoy turning raw data into interactive dashboards and combining statistical precision with modern automation.
            </p>
          </motion.div>
        </div>
        ...
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {statsData.map((stat, idx) => {
            const IconComponent = iconsMap[stat.iconName] || FolderGit2;
            return (
              <motion.div
                key={stat.id}
              ...
              >
                ...
                <div className="text-3xl sm:text-4xl font-bold font-display text-[#1D2A26] tracking-tight mb-1">
                  {stat.numericValue ? (
                    <AnimatedCounter endValue={stat.numericValue} suffix={stat.suffix} />
                  ) : (
                    stat.value
                  )}
                </div>
